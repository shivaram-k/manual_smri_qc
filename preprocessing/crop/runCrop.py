...
and runs the job submission script for each scan to preprocess the scans by cropping them without ACPC alignment.
    ...
    Submits jobs for preprocessing each scan:
        sbatch jobSingleScanPrepocCrop.sh <scanWorkingDir> <scanPath> <newScanPath>
"""
...
def main():
    ...
    # Working directory for cropping 
    ...
    for subID in subIDs:
        ...
        for sesID in sesIDs:
            ...
            for scan in scans:
                ...
                # Submit the job here
                cmd = 'sbatch jobSingleScanPrepocCrop.sh '
                cmd += scanWorkingDir + ' ' + scanPath + ' ' + newScanPath 
                os.system(cmd)
